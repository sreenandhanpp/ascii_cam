...
export default function App() {
  ...
  const [opts, setOpts] = useState<RenderOpts>({ density: 6, fontSize: 8, brightness: 0, contrast: 0.2, saturation: 0.2, gamma: 1, invert: false, mode: 'classic', customCharset: '@#S%?*+;:,. ', edge: 0, sharpen: 0, blur: 0 });
  ...
  return (
    <div className="fixed inset-0 w-screen h-screen overflow-hidden bg-black text-cyan-200 font-mono">
      <AsciiCanvas stream={cam.stream} theme={theme} opts={{ ...opts, mode }} onFrame={onFrame} />
      <header className="absolute top-0 inset-x-0 z-30 p-2 md:p-3 bg-black/35 backdrop-blur-sm border-b border-cyan-500/30">
      ...
      <AnimatePresence>
        {showPanel && (
          <motion.aside initial={{ y: 24, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 24, opacity: 0 }} className="absolute z-30 left-2 right-2 md:left-auto md:w-[340px] md:right-3 top-14 bottom-20 bg-black/75 backdrop-blur border border-cyan-500/30 rounded-xl p-3 overflow-auto space-y-2">
            <select className="ui" value={cam.deviceId} onChange={(e) => cam.setDeviceId(e.target.value)}><option value="">Auto Camera</option>{cam.devices.map((d) => <option key={d.deviceId} value={d.deviceId}>{d.label || 'Camera'}</option>)}</select>
      ...
      <footer className="absolute bottom-0 inset-x-0 z-30 p-2 md:p-3 bg-black/35 backdrop-blur-sm border-t border-cyan-500/30 text-[11px] md:text-xs">{status} | mode:{mode} | res:{res} | rec:OFF</footer>
    </div>
  );
}
